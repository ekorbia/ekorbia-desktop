...
function Message({ m, highlightRegex, chatId, isStreaming, onEditMessage, onRetryMessage, showDetails = false, engineCtx = null }) {
  ...
  // (inspectable) without cluttering the resting view.
  ...
  return (
    ...
    >
      ...
      {/* Model + tokens footer for assistant messages. Progressive
          disclosure (Phase 2 UX polish): shown inline when the global
          "Show technical details" pref is on, otherwise collapsed to a
          quiet "Details ›" affordance that reveals it for THIS message on
          click — the attribution is one click away, never buried. Hidden
          while streaming (the ek-stream-border + stream-cursor already
          indicate live state, and tokens aren't known until `done`). User
      ...
      {!isUser && !m.streaming && (m.model || m.tokens) &&
        (showDetails || detailsOpen ? (
          ...
          <button
            ...
            title="Show model, tokens, and timing"
            style={{
              marginTop: 4,
              marginLeft: 14,
              padding: 0,
...
function Composer({
  ...
  modelHasTools = false,
  // Progressive disclosure (Phase 2 UX polish): when off (default), the
  // green "TOOL" capability chip collapses to a quiet ⓘ beside the model
  // picker whose tooltip lists what the model supports. The capability
  // probe + tool injection are unchanged — this is purely presentational.
  showDetails = false,
          ...
          >
            ...
            {pickerOpen && (
              <ModelPicker
                ...
                // Surfaces "Supports: …" on the active model's row.
                modelHasTools={modelHasTools}
                modelHasVision={modelHasVision}
              />
            )}
            {/* Capability indicator next to the model picker. In the resting
                view capabilities live on the model row inside the picker
                ("Supports: Tool use • Vision" — see ModelPicker). With "Show
                technical details" ON, the explicit green TOOL chip also
                renders here (marks models whose write_file tool calls get
                wired into this chat — see the tool loop in ollama.rs). The
                capability probe + tool injection are unchanged either way.
                Placed outside the picker button so it reads as a state
                indicator, not an action. */}
            {showDetails && modelHasTools && (
              <span
