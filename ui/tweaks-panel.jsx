...
function useTweaks(defaults) {
  // Initialize from localStorage so theme/fontScale/etc. survive across
  // launches. Spread defaults first so any new keys added to TWEAK_DEFAULTS
  // in the future still get their hardcoded default value even when the
  // stored JSON predates them.
  const [values, setValues] = React.useState(() => {
    try {
      const stored = localStorage.getItem('ekorbia.tweaks');
  ...
  const setTweak = React.useCallback((keyOrEdits, val) => {
    ...
    setValues((prev) => {
      ...
      try { localStorage.setItem('ekorbia.tweaks', JSON.stringify(next)); } catch (_) {}
      return next;
    });
