...
- **Spaces — workspaces for related chats** — pick the "+ New Space" button at the top of the sidebar to create a workspace bundling a default model, optional pinned files / folders, optional pinned prompts (any of which can be **locked** so they're always attached and can't be detached per chat), and an optional Space-scoped memory file. New chats created inside a Space inherit all of those automatically — so writing a novel, planning a project, or keeping therapy notes stops being "re-attach the same folder to every chat" and becomes "open the Space, start typing." Drag any chat onto a Space row to file it, or right-click for **Move to Space →**. Hover any Space row for the ⋯ menu: **Edit settings…** / **Rename** / **Change color…** / **Delete**. Chats stay in their Space across launches; click **All chats** at the top to see your full history. A Space with no extra context behaves as a pure folder — Spaces replaced the earlier 0.2.0 "chat groups" concept and existing groups migrate automatically on first launch. See [docs/src/spaces.md](docs/src/spaces.md) for the full walkthrough.
