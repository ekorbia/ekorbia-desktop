...
**Provider seam (no-Ollama plan — Phase 0 seam, Phase 1 dispatch)**: the UI and internal pipelines speak ONLY the neutral surface in `src/llm.rs` — `llm_list_models` / `llm_loaded_models` / `llm_warmup` / `llm_capabilities` / `llm_embed_model_check` / `llm_chat_stream(+_cancel)` commands, plus `llm::chat` / `llm::embed` / `llm::model_has_vision` for the watch + attachment pipelines. Adapters live in `src/providers/`: `ollama.rs` (default engine; ALL Ollama HTTP) and `openai_compat.rs` (any /v1 server — LM Studio, llama-server, vLLM; translates the Ollama-shaped request body and its SSE responses). `providers/mod.rs` owns the shared cancel registry. Dispatch: `llm.rs` BackendConfig from settings keys `llm_backend` ("ollama" default | "openai"), `llm_base_url` (Ollama: optional override; OpenAI: REQUIRED, normalized — no trailing slash or `/v1`), `llm_api_key` — loaded at startup, switched live by `llm_backend_config_set` (Settings → Backend tab, `BackendSettings` in settings.jsx). The streaming channel carries the tagged `StreamEvent` contract (`{type: delta|toolCalls|done|error}`, promptTokens/outputTokens on done); EVERY adapter must uphold the pinned guarantees (each has golden tests): `toolCalls` at most once per stream, before `done`; `function.arguments` ALWAYS a JSON object; the UI's three consumers (handleSend, compare fan-out, overlay) switch on `ev.type` and never see provider-raw chunks. BYO capability defaults are deliberate (tools ON, vision/thinking OFF — no portable probe; see openai_compat docs). Ollama-*lifecycle* commands (`start_ollama`, `ollama_pull(_cancel)`, `ollama_delete`) keep their names — they manage that engine's install, not "an LLM" — and the UI hides them on BYO. The IPv4 `OLLAMA_BASE` regression test is unchanged.
